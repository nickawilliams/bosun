...
func selectReviewTargets(ctx context.Context, cmd *cobra.Command, host code.Host, g vcs.VCS, resolved []repoContext) error {
	...
	// the session shell the form embeds beneath the "pull requests"
	// input header in the same program — the formFirstFrame takeover
	// that used to bridge the steps program into huh's program is gone.
	...
	// on — the "pull requests" input header when the selection form
	// follows (silent in raw mode: the record card would misreport
	// before the form adjusts the selection), or the record card when
	// no form will show.
...
func newReviewCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return ui.RunSession(func() error { return runReview(cmd) })
		},
	}
...
// runReview is the review command body, hosted by the session shell.
